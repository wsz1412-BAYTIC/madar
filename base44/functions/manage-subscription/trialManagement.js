...
// Reminder schedule: days-before-expiry. 0 = on the expiry day itself.
 ...
 * `sentLog` is the subscription's trialRemindersSent array (e.g. ['7d']).
 * An offset is due when now >= (trialEndsAt − offset days) and the trial is
 * still relevant (active, or expired only for the day-0 notice). Each offset
 * fires at most once — that is the dedupe guarantee.
 */
export function dueTrialReminders(subscription, now = new Date(), sentLog = null) {
  ...
  for (const offset of REMINDER_OFFSETS_DAYS) {
    ...
    // Don't fire pre-expiry offsets after the trial already ended (the day-0
    // notice covers that), and never fire anything more than a day past expiry.
