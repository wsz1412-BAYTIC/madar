...
// by impact, and return ONLY the top 3 fixes for trial/free customers (simple
// language, expected benefit included). A verified paid subscription unlocks
// the full list. Mirrored into base44/functions/manage-subscription/.

export const QUICK_REPORT_LIMIT = 3;

// Each detector returns null (no issue) or an issue instance.
// weight = ranking impact (higher = more important).
const DETECTORS = [
  {
    id: 'no_photos',
    weight: 90,
  ...
  },
  {
    id: 'no_price',
    weight: 70,
    test: (p) => !(Number(p.nightlyPrice) > 0) && !(Number(p.price) > 0),
    issue: (names) => ({
  ...
  },
  {
    id: 'paused',
    weight: 50,
    test: (p) => p.status === 'inactive' || p.status === 'paused',
    issue: (names) => ({
...
const displayName = (p) => p.name || p.nameAr || 'property';

/**
 * Build the quick report.
 * @param properties  the user's UserProperty rows (may be empty)
 * @param opts.fullAccess  true when the user has a verified PAID subscription —
 *                         unlocks the complete issue list; otherwise top 3 only.
 * Returns { issues, totalIssues, lockedCount, fullAccess, summary:{en,ar} }.
 */
