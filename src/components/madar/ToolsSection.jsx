    ...
    <section className="py-28 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <FadeIn className="text-center mb-20">
          ...
          <p className="text-[#F7F5F0]/50 text-lg max-w-2xl mx-auto leading-relaxed">
            {t('toolsSubtitle')}
          </p>
        </FadeIn>
        ...
        <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5" stagger={0.08}>
          {tools.map((tool) => (
            <StaggerItem key={tool.titleKey}>
              <div className="group h-full p-7 rounded-3xl glass hover:border-white/15 hover:bg-white/[0.06] transition-all duration-500 hover:-translate-y-1">
                ...
                <p className="text-[#F7F5F0]/50 text-sm leading-relaxed">{t(tool.descKey)}</p>
              </div>
            </StaggerItem>
          ))}
        </StaggerContainer>
