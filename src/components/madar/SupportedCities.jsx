...
export default function SupportedCities() {
  ...
  return (
    <section className="relative py-28 px-4 sm:px-6 lg:px-8 bg-[#EFF6FA] overflow-hidden">
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-[#ADDFF1]/5 rounded-full blur-[120px] pointer-events-none" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {cities.map((city, i) => (
            <motion.div
              ...
              className="group rounded-2xl bg-[#06131F] border border-white/[0.06] hover:border-white/15 p-6 transition-all duration-500 hover:-translate-y-1"
            >
              <div className="flex items-start justify-between mb-5">
                <div>
                  <h3 className="font-heading text-xl font-bold text-[#F2F8FC]">
                    {lang === 'ar' ? city.nameAr : city.name}
                  </h3>
                  <span className="text-xs text-[#F2F8FC]/30">
                    {city.properties} {lang === 'ar' ? 'عقار' : 'properties'}
                  </span>
                </div>
                <div className="w-9 h-9 rounded-lg glass flex items-center justify-center">
                  <MapPin className="w-4 h-4 text-[#0F6BA8]" />
                </div>
              </div>
              ...
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] text-[#F2F8FC]/40">{lang === 'ar' ? 'مؤشر الطلب' : 'Demand Index'}</span>
                  <span className="text-sm font-bold text-[#0F6BA8]">{city.demand}%</span>
                </div>
                <div className="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                  <motion.div
                    ...
                    className="h-full rounded-full bg-gradient-to-r from-[#00548C] to-[#003152]"
                  />
                </div>
              </div>

              <div className="flex items-center justify-between pt-4 border-t border-white/[0.06]">
                <span className="text-[11px] text-[#F2F8FC]/40">{lang === 'ar' ? 'متوسط السعر' : 'Avg Rate'}</span>
                <span className="text-sm font-medium text-[#F2F8FC]/70">{city.avgRate}</span>
              </div>
            </motion.div>
          ))}
        </div>
