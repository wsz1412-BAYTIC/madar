        ...
        <div className="max-w-4xl mx-auto text-center">
          <FadeIn>
            <div className="flex justify-center gap-4 mb-6">
              <Copyright className="w-12 h-12 text-[#D95F3B]" />
              <Award className="w-12 h-12 text-[#C8972A]" />
            </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-4xl mx-auto">
          <FadeIn className="mb-12">
            ...
            <p className="text-[#0A0B10]/60 leading-relaxed mb-8">
              {lang === 'ar'
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {madarOwned.map((item, i) => (
                <FadeIn key={i} delay={i * 0.05}>
                  <div className="p-5 rounded-xl bg-[#F2EFE8] border border-[#D95F3B]/20 flex items-start gap-3">
                    <Copyright className="w-4 h-4 text-[#D95F3B] flex-shrink-0 mt-1" />
                    <span className="text-[#0A0B10]/70 text-sm">{lang === 'ar' ? item.itemAr : item.item}</span>
                  </div>
                </FadeIn>
              ))}
            </div>
          ...
          <FadeIn>
            <div className="p-8 rounded-2xl bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200">
              <h3 className="font-heading font-bold text-[#0A0B10] text-lg mb-4">
                {lang === 'ar' ? 'تقييدات الاستخدام' : 'Usage Restrictions'}
              </h3>
              <ul className="space-y-3 text-[#0A0B10]/70 text-sm">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
            {thirdParty.map((item, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <div className="p-6 rounded-2xl bg-white border border-[#0A0B10]/[0.06] text-center">
                  <p className="font-heading font-bold text-[#0A0B10] text-lg">
                    {item.name}
                    <span className="text-[#D95F3B] ml-1">{item.symbol}</span>
                  </p>
                  <p className="text-xs text-[#0A0B10]/50 mt-2">
      ...
      <footer className="py-16 px-4 border-t border-[#0A0B10]/[0.06] bg-[#F2EFE8]">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-8">
            ...
            <a href="/ip-trademark" className="text-sm text-[#0A0B10]/40 hover:text-[#D95F3B] transition-colors">
              {lang === 'ar' ? 'الملكية الفكرية' : 'IP'}
            </a>
