    ...
    <div className="space-y-8 max-w-2xl">
      <FadeIn>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-heading text-3xl font-bold text-[#F7F5F0]">{t('referralProgram')}</h1>
            <p className="text-sm text-[#F7F5F0]/40">{t('referralDesc')}</p>
          </div>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.1}>
        <div className="relative rounded-2xl p-6 overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-white/15 backdrop-blur rounded-xl px-4 py-3 font-mono text-lg font-bold tracking-wider text-white">
                {code}
              </div>
              <button onClick={handleCopy} className="flex items-center gap-2 px-4 py-3 bg-white/20 rounded-xl hover:bg-white/30 transition-all text-sm font-medium text-white">
        ...
        {[
          ...
          <FadeIn key={stat.key} delay={0.15 + i * 0.08}>
            <div className="glass rounded-2xl p-5">
              ...
              <div className="text-lg font-bold text-[#F7F5F0] font-heading">{stat.value}</div>
              <div className="text-xs text-[#F7F5F0]/40 mt-0.5">{t(stat.key)}</div>
            </div>
          </FadeIn>
        ))}
      </div>
      ...
      <FadeIn delay={0.3}>
        <div className="glass rounded-2xl p-6">
          <h2 className="font-heading font-semibold text-[#F7F5F0] mb-4">{lang === 'ar' ? 'الإحالات الأخيرة' : 'Recent Referrals'}</h2>
          <div className="space-y-3">
            {referrals.map((ref, i) => (
              <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.04] hover:border-white/10 transition-all">
                <div>
                  <div className="font-medium text-sm text-[#F7F5F0]">{ref.name}</div>
                  <div className="text-xs text-[#F7F5F0]/30">{ref.date}</div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  {ref.earned > 0 && <span className="text-sm font-semibold text-[#F7F5F0]">{ref.earned} {sar}</span>}
                </div>
              </div>
            ))}
          </div>
