...
// string). Mirrored into base44/functions/*/ (see functionMirrors.test.js).

const round2 = (v) => Math.round((v + Number.EPSILON) * 100) / 100;

export const DEFAULT_PLATFORM_FEES = Object.freeze({
  'Airbnb': 0.14, // host + guest fee blended estimate
  'Gathern': 0.125,
  'Booking.com': 0.15,
  'Other': 0.10,
});
 ...
 * bad config must never break pricing.
 */
 ...
 * Returns { platform, rate, estimated, source } where source is
 * 'config' (confirmed override), 'default' (bundled estimate), or
 * 'default_other' (platform unknown → Other estimate).
 */
 ...
 * Returns { gross, feeRate, feeAmount, net, estimated, source, platform }
 * or null when gross is not a usable number.
 */
...
/**
 * Deterministic revenue projection for a nightly-price change. No guarantees —
 * a straight-line estimate at unchanged occupancy, clearly labeled as such.
